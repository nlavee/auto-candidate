...
Uses the 'claude' CLI command for all interactions.
"""
import os
import json
import subprocess
...
class ClaudeProvider(BaseLLMProvider):
    ...
    def _call_cli(self, prompt: str, system_instruction: str = "") -> str:
        ...
        try:
            ...
            console.print(f"[bold red]Claude CLI Error:[/bold red] {e.stderr}")
            raise

    @staticmethod
    def list_available_models(api_key: str) -> list[str]:
        """Lists available Claude models."""
        # Claude doesn't have a dynamic model listing API via CLI
    ...
    def create_task_breakdown(self, challenge_prompt: str, codebase_context: str) -> dict:
        ...
        """
        console.print(f"[cyan]Formulating task-based plan via Claude ({self.model_name})...[/cyan]")
    ...
    def execute_task(self, task: dict, codebase_context: str, plan_overview: str = "", task_spec: str = "") -> str:
        ...
        try:
            ...
            console.print(f"[bold red]Claude Execution Error during coding:[/bold red] {e}")
            return ""
    ...
    def fix_code(self, error_log: str, original_task: str, codebase_context: str) -> str:
        ...
        try:
            ...
            console.print(f"[bold red]Claude Execution Error during debugging:[/bold red] {e}")
            return ""
    ...
    def resolve_conflict(self, conflict_info: dict, plan_context: str = "") -> str:
        ...
        try:
            ...
            console.print(f"[bold red]Claude Execution Error during conflict resolution:[/bold red] {e}")
            return ""
    ...
    def generate_plans(self, challenge_prompt: str, codebase_context: str, num_versions: int = 2) -> list[str]:
        ...
        """
        console.print(f"[cyan]Contacting Claude CLI ({self.model_name}) to formulate plans...[/cyan]")
    ...
    def generate_code(self, plan: str, codebase_context: str) -> str:
        ...
        Returns the raw LLM response containing file blocks.
        """
        console.print(f"[cyan]Generating code via Claude CLI ({self.model_name})...[/cyan]")
